...
export function ChargeConceptsPanel({ concepts: initial }: { concepts: Concept[] }) {
  ...
  // COLUMNS dentro del componente: la celda de acciones usa handleDelete/pending vía closure.
  const COLUMNS: Column<Concept>[] = [
    ...
    { key: "name", header: "Nombre", card: "title", cell: (c) => c.name },
    {
      key: "category",
      header: "Categoría",
      card: "hidden",
  ...
  return (
    <div className="space-y-3">
      ...
      <DataTable
        ...
        rows={filtered}
        getRowKey={(c) => c.id}
        caption="Conceptos de cargo"
        empty={concepts.length === 0 ? "Sin conceptos aún. Crea el primero." : "Sin resultados"}
      />
    </div>
  );
}
